...
const getBlogs = async function (req, res) {
    try {
        ...
        if (getSpecificBlogs.length == 0) {
            return res.status(400).send({ status: false, msg: 'No blogs can be found' });
        }
        else {
...
// If it doesn't, return an HTTP status 404 
    ...
    try {
        ...
        if (!savedData.isDeleted) {
            if (savedData.isPublished == true) {
                ...
                res.status(201).send({ status: true, message:'updated data successfully', data: { getSpecificBlogs } })
            }
...
// Delete blog documents by category, authorid, tag name, subcategory name, unpublished // means (published:false)
...
let deleteBlog = async function (req, res) {
    ...
    try {
        ...
        //when authorId is present in data 
        if (data["authorId"]) {
            ...
            filter["authorId"] = data["authorId"]  //(created authorId in filter)
        ...
        } //when authorId is not present in data, create authorId key in filter & take value from req.authorLoggedIn.authorId(from verified token)
        else {
        ...
        res.status(200).send({ status: true, msg: savedData , msg: "deleted sucessfully"});
    }
    catch (error) {
...
module.exports = { createBlogs, getBlogs, updatedBlogs, deleteBlog, deleteBlogById }
